...
# GNU first: Linux `stat -f` is --file-system (not BSD format) and pollutes stdout.
perms="$(stat -c '%a' "$SSH_KEY_RUNTIME" 2>/dev/null || stat -f '%Lp' "$SSH_KEY_RUNTIME")"
assert_eq "600" "$perms" "runtime key mode 600"
runtime_perms="$(stat -c '%a' "$RUNTIME_DIR" 2>/dev/null || stat -f '%Lp' "$RUNTIME_DIR")"
