...
Local TLS for [Coolify](https://coolify.io) + Traefik, using a persistent [mkcert](https://github.com/FiloSottile/mkcert) CA.

**For private LAN / VPN / homelab only** — not a public CA and not a replacement for Let’s Encrypt. Read [SECURITY.md](./SECURITY.md) before enabling SSH sync or distributing `rootCA.pem`.
...
Uncomment the single-key volume in compose. Prefer a dedicated sync user — [docs/ssh-least-privilege.md](./docs/ssh-least-privilege.md).
...
| Browsers still warn | Client has not trusted `rootCA.pem`; wrong leaf SANs |
